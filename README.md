...
| 2 | フードアレルギーセーフティスキャナー — メニュー情報表示 | ✅ 作成済み | `AllergyGuardMock` |
| 3 | AR タイムトラベラー — 過去の姿を現在に重ねて表示 | ✅ 作成済み | `ARTimeTravelerMock` |
| 4 | ライブ投票・インタラクション — コンサート会場NFC投票 | ✅ 作成済み | `LiveVotingMock` |
| 5 | 緊急医療情報カード — NFCブレスレットで医療情報表示 | ✅ 作成済み | `EmergencyMedicalCardMock` |

### App Intents

| # | アイデア | 状態 | プロジェクト |
|---|---------|------|------------|
| 1 | AI 習慣コーチ — Siriが生活パターンを学習して提案 | ✅ 作成済み | `HabitCoachMock` |
| 2 | 声だけ料理アシスタント — ハンズフリー操作レシピ | ✅ 作成済み | `VoiceRecipeMock` |
| 3 | Spotlight で完結するミニ家計簿 — アプリ不要の家計管理 | ✅ 作成済み | `KakeiboMock` |
| 4 | コンテキスト DJ — 状況に応じた自動プレイリスト生成 | ✅ 作成済み | `ContextDJMock` |
| 5 | 学習カード・エブリウェア — システムに溶け込むフラッシュカード | ✅ 作成済み | `FlashCardEveryWhereMock` |

### ARKit

| # | アイデア | 状態 | プロジェクト |
|---|---------|------|------------|
| 1 | AR タイムマシン — 過去と未来の街並みを歩く | ✅ 作成済み | `ARTimeMachineMock` |
| 2 | AR 生き物図鑑 — 部屋が水族館・動物園になる | ✅ 作成済み | `NazoWalkMock` |
| 3 | AR シャドウ — 光と影の物理パズルゲーム | ✅ 作成済み | `ARShadowMock` |
| 4 | AR ミュージアム — 自分だけの美術館を建築 | ✅ 作成済み | `ARMuseumMock` |
| 5 | AR フィットネスコーチ — モーションキャプチャでフォーム改善 | ✅ 作成済み | `ARFitnessCoachMock` |

### AVKit / AVFoundation

| # | アイデア | 状態 | プロジェクト |
|---|---------|------|------------|
| 1 | CineMagic — AI リアルタイム映画フィルター撮影 | ✅ 作成済み | `CineMagicMock` |
...
| # | アイデア | 状態 | プロジェクト |
|---|---------|------|------------|
| 1 | AI フォーム改善コーチ — リアルタイム運動フォーム分析 | ✅ 作成済み | `AIFormCoachMock` |
| 2 | マルチモーダル食事記録 — 写真から自動栄養素記録 | ✅ 作成済み | `FoodLogAIMock` |
| 3 | リアルタイム環境音翻訳機 — 周囲の音を認識 | ✅ 作成済み | `SoundTranslatorMock` |
| 4 | 手書きノート AI アシスタント — 手書きメモを構造化 | ✅ 作成済み | `HandwritingAIMock` |
